...
case _SC( '\t' ): case _SC( '\r' ): case _SC( ' ' ): NEXT(); continue;
		...
		case _SC( '=' ):
			...
			if ( CUR_CHAR != _SC( '=' ) ) {
				...
				NEXT(); RETURN_TOKEN( TK_EQ );
			}
		case _SC( '<' ):
						NEXT();
			if ( CUR_CHAR == _SC( '=' ) ) {
				NEXT(); RETURN_TOKEN( TK_LE )
			} else if ( CUR_CHAR == _SC( '-' ) ) {
				NEXT(); RETURN_TOKEN( TK_NEWSLOT );
			} else if ( CUR_CHAR == _SC( '<' ) ) {
				NEXT(); RETURN_TOKEN( TK_SHIFTL );
			} else if ( CUR_CHAR == _SC( '/' ) ) {
				NEXT(); RETURN_TOKEN( TK_ATTR_OPEN );
			}
...
case _SC( '{' ): case _SC( '}' ): case _SC( '(' ): case _SC( ')' ): case _SC( '[' ): case _SC( ']' ):
		...
		case _SC( ':' ):
			...
			if ( CUR_CHAR != _SC( ':' ) ) {
				...
				NEXT(); RETURN_TOKEN( TK_DOUBLE_COLON );
			}
		case _SC( '*' ):
						NEXT();
			if ( CUR_CHAR == _SC( '=' ) ) {
				NEXT(); RETURN_TOKEN( TK_MULEQ );
			} else RETURN_TOKEN( '*' );
		case _SC( '%' ):
						NEXT();
			if ( CUR_CHAR == _SC( '=' ) ) {
		...
		case _SC( '-' ):
						NEXT();
			if ( CUR_CHAR == _SC( '=' ) ) {
				NEXT(); RETURN_TOKEN( TK_MINUSEQ );
			} else if  ( CUR_CHAR == _SC( '-' ) ) {
				NEXT(); RETURN_TOKEN( TK_MINUSMINUS );
			} else RETURN_TOKEN( '-' );
		case _SC( '+' ):
						NEXT();
			if ( CUR_CHAR == _SC( '=' ) ) {
				NEXT(); RETURN_TOKEN( TK_PLUSEQ );
			} else if ( CUR_CHAR == _SC( '+' ) ) {
				NEXT(); RETURN_TOKEN( TK_PLUSPLUS );
			} else RETURN_TOKEN( '+' );
		case SQUIRREL_EOB:
			return 0;
		default: {
