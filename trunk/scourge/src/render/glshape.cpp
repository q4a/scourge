...
void GLShape::createDarkTexture( WallTheme *theme ) {
  ...
  */
  ...
  gluBuild2DMipmaps(GL_TEXTURE_2D, 4, LIGHTMAP_SIZE, LIGHTMAP_SIZE, GL_RGBA, GL_UNSIGNED_BYTE, data);
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
  gluBuild2DMipmaps(GL_TEXTURE_2D, 4, LIGHTMAP_SIZE, LIGHTMAP_SIZE, GL_RGBA, GL_UNSIGNED_BYTE, data2);
  glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);
  glTexImage2D(GL_TEXTURE_2D, 0, 3, LIGHTMAP_SIZE, LIGHTMAP_SIZE, 0, 
