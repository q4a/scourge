...
void GLShape::drawHeightMap( float ground[][MAP_DEPTH], int groundX, int groundY ) { 
  ...
  if( tex && tex[ textureIndex ]) glBindTexture( GL_TEXTURE_2D, tex[ textureIndex ] );
	...
	glVertex3f( 0, depth / DIV, 2.0f / DIV );
	glVertex3f( 0, 0, 2.0f / DIV );
	glVertex3f( width / DIV, 0, 2.0f / DIV );
	glVertex3f( width / DIV, depth / DIV, 2.0f / DIV );
	glEnd();
	glDepthMask( GL_TRUE );
	*/
...
float GLShape::getLight( float *normal ) {
