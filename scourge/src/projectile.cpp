...
#endif
            session->getMap()->addDescription( _( "Projectile did not reach the target (max steps)." ) );
            removedProjectiles.push_back(proj);
          }
...
#endif
                // hit something
                session->getMap()->addDescription( _( "Projectile did not reach the target (blocked)." ) );
                blocked = true;
              }
            }
          }
