...
public class Window implements NodeGenerator, MouseInputListener {
    ...
    public Window(int x, int y, int w, int h, WindowListener listener) {
        ...
        Quad q = new Quad(ShapeUtil.newShapeName("ground"), w, h);
        ...
        Texture texture = ShapeUtil.loadTexture(WINDOW_BACKGROUND);
    ...
    public void addLabel(int x, int y, String text, float size, int flags, ColorRGBA color, float scale) {
        Text2D label = createLabel(x, y, text, size, flags, color, scale);
		win.attachChild(label);
    }

    Text2D createLabel(int x, int y, String text, float size, int flags, ColorRGBA color, float scale) {
